...
import { CreatePost, type ServiceContract } from "./Post.types";
...
export const postService: ServiceContract = {
	getSplicedPosts: async (skip, take, filter) => {
		...
		const filteredPosts = await productRepository.getSplicedPosts(
			numberSkip,
			numberTake,
			boolFilter,
		);
	...
	updateDataPost: async (postId, postData, token) => {
		const post = await productRepository.updateDataPost(
			postId,
			postData,
			token,
		);
	...
	getPostWithRels: async (postId) => {
		const post = await productRepository.getPostWithRels(postId);
		return post;
	},
    isLikedPost: async (postId, token) => {
		const response = await productRepository.isLikedPost(postId, token);
		return response;
	},
};
